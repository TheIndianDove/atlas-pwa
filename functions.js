...
function buildDefaultScheduleV2(programKey) {
  ...
  // Evenly distribute rest days
  const restMask = new Array(7).fill(false);
  if (restLimit > 0) {
    const step = 7 / restLimit; const used = new Set();
...
function deriveWeekLabels(state) {
  ...
  let k = 0;
  for (let i = 0; i < 7; i++) {
    if (restMask[i]) labs[i] = "Rest";
    else { labs[i] = rotation[(startOffset + k) % rotation.length]; k++; }
  }
  return labs;
}

function initSchedule() {
  const root = document.getElementById("scheduleRoot"); if (!root) return;
  ...
  const weekEl = document.getElementById("weekContainer");
  ...
  let state = getScheduleV2(programKey);
  let labels = deriveWeekLabels(state);
  programInfo.textContent = `Program: ${prettyProgramName(programKey)}`;

  const restCount = (mask = state.restMask) => mask.filter(Boolean).length;
  const updateCounter = () => { restCounter.textContent = `Rest Days Used: ${restCount()} / ${state.restLimit}`; };

  const undoStack = [];
  const pushUndo = (prev) => undoStack.push(JSON.stringify(prev));
  const restoreUndo = () => {
    const raw = undoStack.pop(); if (!raw) return;
    state = JSON.parse(raw); labels = deriveWeekLabels(state); saveScheduleV2(state); buildWeek();
  };

  function attachTapHandlers(card, i) {
    let lastTap = 0; const TAP = 300;
    card.addEventListener("click", (ev) => {
      const now = Date.now();
      if (now - lastTap < TAP) { lastTap = 0; ev.preventDefault(); onDoubleTap(i); }
      else {
        lastTap = now;
        setTimeout(() => { if (Date.now() - lastTap >= TAP) onSingleTap(i); }, TAP + 10);
      }
    });
  }
  function onSingleTap(i) {
    if (state.restMask[i]) return;
    ...
    saveScheduleV2(state); labels = deriveWeekLabels(state); buildWeek(); pushUndo(prev);
    showToast(state.completed.includes(i) ? "Marked completed" : "Marked not completed", { actionLabel: "Undo", onAction: restoreUndo });
  }
  function onDoubleTap(i) {
    const prev = deepClone(state);
    if (!state.restMask[i]) {
      if (restCount() >= state.restLimit) { showToast(`Max ${state.restLimit} rest days reached`); return; }
      state.restMask[i] = true;
      const cix = state.completed.indexOf(i);
      if (cix !== -1) state.completed.splice(cix, 1);
    } else state.restMask[i] = false;
    saveScheduleV2(state); labels = deriveWeekLabels(state); buildWeek(); pushUndo(prev);
    showToast("Day updated", { actionLabel: "Undo", onAction: restoreUndo });
  }

  function swapDay(i, j) {
    const t = state.restMask[i]; state.restMask[i] = state.restMask[j]; state.restMask[j] = t;
    const hasI = state.completed.includes(i), hasJ = state.completed.includes(j);
    if (hasI && !hasJ) { state.completed.splice(state.completed.indexOf(i), 1); state.completed.push(j); }
    else if (hasJ && !hasI) { state.completed.splice(state.completed.indexOf(j), 1); state.completed.push(i); }
  }

  function setupDrag(cardEl, i) {
    let dragging = false, from = i, overIndex = i, pressTimer = null;
    const cards = () => Array.from(weekEl.children);
    ...
    const end = () => {
      ...
      if (!dragging) return; dragging = false; cardEl.classList.remove("dragging");
      if (overIndex !== from) {
        const prev = deepClone(state); swapDay(from, overIndex); saveScheduleV2(state); buildWeek();
        showToast("Reordered", {
          actionLabel: "Undo",
          onAction: () => { state = deepClone(prev); saveScheduleV2(state); buildWeek(); }
        });
      }
    };
    ...
    cardEl.addEventListener("pointermove", (e) => {
      if (!dragging) return;
      const y = e.clientY; let nearest = 0, best = Infinity;
      cards().forEach((c, idx) => {
        const r = c.getBoundingClientRect(); const cy = r.top + r.height / 2;
        const d = Math.abs(cy - y); if (d < best) { best = d; nearest = idx; }
      });
      overIndex = nearest;
    }, { passive: true });

    ["pointerup", "pointercancel", "pointerleave"].forEach((ev) => cardEl.addEventListener(ev, end));
  }

  function buildWeek() {
    weekEl.innerHTML = ""; labels = deriveWeekLabels(state); updateCounter();
    for (let i = 0; i < 7; i++) {
      const card = tpl.content.firstElementChild.cloneNode(true);
      card.dataset.index = String(i);
      card.querySelector(".day-name").textContent = DAYN[i];
      card.querySelector(".slot-label").textContent = labels[i];
      const isRest = state.restMask[i], isDone = state.completed.includes(i);
      card.classList.toggle("rest", isRest); card.classList.toggle("completed", isDone);
      card.querySelector(".type-text").textContent = isRest ? "Rest day (double-tap to switch)" : "Tap to toggle complete";
      card.querySelector(".complete-text").textContent = (isDone && !isRest) ? "Completed" : "";
      attachTapHandlers(card, i);
      setupDrag(card, i);
  ...
  editBtn?.addEventListener("click", () => {
    modeBadge?.classList.toggle("hidden");
    showToast("Edit: double-tap Rest • long-press to drag");
  });
  resetDefaultBtn?.addEventListener("click", () => {
    if (!confirm("Reset to default schedule?")) return;
    const prev = deepClone(state);
    state = buildDefaultScheduleV2(programKey); saveScheduleV2(state); buildWeek();
    showToast("Schedule reset", { actionLabel: "Undo", onAction: () => { state = prev; saveScheduleV2(state); buildWeek(); } });
  });
  resetProgressBtn?.addEventListener("click", () => {
    if (!confirm("Reset this week’s progress?")) return;
    const prev = deepClone(state); state.completed = []; saveScheduleV2(state); buildWeek();
    showToast("Progress cleared", { actionLabel: "Undo", onAction: () => { state = prev; saveScheduleV2(state); buildWeek(); } });
  });
...
function initTracker() {
  ...
  function refreshDatalist() {
    ...
    if (exact) {
      hint.innerHTML = `Selected: ${exact.n}<br>${exact.kcal} kcal / 100g  (P:${exact.p} F:${exact.f} C:${exact.c})`;
    } else {
